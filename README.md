...
# 3) Training custom dataset
   * Use the following command to start training on custom dataset. 
   !python train.py --weights yolov7.pt --data "data/custom.yaml" --workers 4 --batch-size 4 --img 416 --cfg cfg/training/yolov7.yaml --name yolov7 --hyp data/hyp.scratch.p5.yaml --epochs 5     change the parameters as required
# 4) For evaluation 
